...
Los canales tienen IDs deterministas `guard_shifts_v1_<hash-del-sonido>`. Al usar un sonido distinto se crea el canal correspondiente y se retiran únicamente canales antiguos propiedad de esta función. La URI elegida por el selector oficial se conserva como metadato; no se copia audio ni se solicita acceso general a archivos.
