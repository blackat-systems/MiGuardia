...
Pulso Vigilia usa canales deterministas `guard_shifts_v3_<hash-del-modo-y-sonido>`. El modo global puede ser sonido y vibración, sólo vibración o silencioso. El sonido participa del hash únicamente cuando corresponde reproducirlo. Al cambiar la combinación se crea el canal coherente y se retiran únicamente canales antiguos con el prefijo propio `guard_shifts_`. La URI elegida por el selector oficial se conserva como metadato; no se copia audio ni se solicita acceso general a archivos. Android y el usuario conservan el control final del canal, sonido y vibración.

La configuración global reconoce tres ritmos por su combinación efectiva, sin persistir una segunda fuente de verdad: `Acompañado` (12 h y 2 h, fijo, completo, sonido y vibración), `Esencial` (12 h, fijo, completo, sonido y vibración) y `Discreto` (12 h, descartable, reducido y silencioso). Cualquier modificación deriva `Personalizado`. La ausencia de la clave nueva mantiene `Esencial`, por lo que no existe migración de preferencias ni cambio silencioso para usuarios existentes.

La pantalla muestra una vista previa Compose con datos ficticios y puede publicar una notificación de prueba independiente con identidad reservada. Esa prueba no usa UUID de guardia, alarmas, agrupación, acciones, rastreo de avisos ni repositorios; respeta privacidad y modo de atención y vence automáticamente al minuto.

El receptor vuelve a consultar el ocultamiento tanto antes como después de publicar. La transición a visible se registra atómicamente sólo si la identidad no está ocultada; si un descarte compite con la publicación o con un refresco tardío de clima, se cancela nuevamente. Las fronteras `REMINDER` y `START` conservan su programación, pero no pueden reactivar una guardia ocultada.
